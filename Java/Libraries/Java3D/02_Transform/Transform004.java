...
public class Transform004 {
	public static void main(String[] args){
		new Transform004("ななめっちゃったけど君は間違ってない、自身を持て四角き物");
	}
	...
	private TransformGroup getWorldTransform(){ /*{{{*/
		...
		world.setTransform( ((Supplier<Transform3D>)() -> {
			...
			first.setTranslation( new Vector3f(0.4f, 0.0f, 0.0f) );
			first.rotY(Math.PI/4);
			first.setScale(0.5f);

			Transform3D second = new Transform3D();
			second.rotX(Math.PI/3);
			second.mul(first);  // firstとsecondの設定をマージ
			return second;
		}).get() );
	 ...
	 */
	public Transform004(String title){ /*{{{*/
